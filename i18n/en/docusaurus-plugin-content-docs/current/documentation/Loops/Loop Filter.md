...
![Image](@site/static/img/docs/Loops/loop-filter-1.png)

Example B: Make a new loop by repeating a single value X times.

示例 B：通过将单个值重复 X 次来创建一个新循环。

![Image](@site/static/img/docs/Loops/loop-filter-2.png)

Example C: Repeat and filter values by using a loop of numbers.

示例 C：使用数字循环重复和过滤值。

![Image](@site/static/img/docs/Loops/loop-filter-3.png)
...
![Image](@site/static/img/docs/Loops/loop-filter.png)
...
![Image](@site/static/img/docs/Loops/loop-filter-item.png)
